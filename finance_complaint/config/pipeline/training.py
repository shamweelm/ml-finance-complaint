...
class FinanceConfig:
    ...
    def get_data_ingestion_config(self, from_date=DATA_INGESTION_MIN_START_DATE, to_date=None) \
        ...
        print("from_date_obj", from_date_obj)
        # If from_date is less than min_start_date, then from_date is set to min_start_date
        if from_date_obj < min_start_date:
            from_date = DATA_INGESTION_MIN_START_DATE
        # If to_date is not provided, then to_date is set to current date
        if to_date is None:
        ...
        Metadata information and ingested file is saved in this directory to avoid duplicate ingestion
        """
